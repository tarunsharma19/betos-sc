...
var AnsClient = class {
  ...
  async getAddressByName(name) {
    ...
    if (!domain)
      return null;
    if (subdomain)
      return this.getAddressBySubdomainName(domain, subdomain);
  ...
  async mintAptosName(account, domainName, years = 1, extraArgs) {
    if (domainName.match(nameComponentPattern) === null) {
      throw new ApiError2(400, `Name ${domainName} is not valid`);
    }
    const address = await this.getAddressByName(domainName);
    if (address !== null) {
      throw new ApiError2(400, `Name ${domainName} is not available`);
    }
  ...
  async getAddressByDomainName(domain) {
  ...
  async getAddressBySubdomainName(domain, subdomain) {
    if (domain.match(nameComponentPattern) === null)
      return null;
