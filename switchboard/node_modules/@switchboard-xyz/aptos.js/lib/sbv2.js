...
import { Account, Hex, } from "@aptos-labs/ts-sdk";
 ...
 * @returns
 */
export async function sendAptosTx(client, signer, method, args, type_args = [], maxGasPrice = 2000) {
 ...
 * @returns
 */
export async function sendSimpleAptosTx(client, signer, simple_tx, maxGasPrice = 2000) {
    try {
        const simulation = (await client.transaction.simulate.simple({
            signerPublicKey: signer.publicKey,
            transaction: simple_tx,
 ...
 * @returns
 */
export function getAptosTx(aptosClient, sender, method, args, type_args = []) {
...
export const sendRawAptosTx = sendAptosTx;
/**
...
export class AptosEvent {
    ...
    async onTrigger(callback, errorHandler) {
        ...
        const deprecatedClient = new AptosClient(this.client.config.network);
...
export class AggregatorAccount {
    ...
    static async init(client, account, params, switchboardAddress) {
        const { mantissa: vtMantissa, scale: vtScale } = AptosDecimal.fromBig(params.varianceThreshold ?? new Big(0));
        const seed = params.seed ? params.seed : Account.generate().accountAddress;
    ...
    async saveResult(account, params, maxGasPrice = 400) {
        ...
        return sendRawAptosTx(this.client, account, `${this.switchboardAddress}::aggregator_save_result_action::run`, [
            BCS.bcsToBytes(TxnBuilderTypes.AccountAddress.fromHex(params.oracleAddress.toString())),
            BCS.bcsToBytes(TxnBuilderTypes.AccountAddress.fromHex(this.address.toString())),
            ...
            BCS.bcsSerializeBytes(Hex.fromHexString(params.jobsChecksum).toUint8Array()),
...
export class JobAccount {
    ...
    async loadJob() {
        ...
        const job = OracleJob.decodeDelimited(Buffer.from(Buffer.from(data.data).toString(), "base64"));
        return job;
    }
    /**
...
export class OracleAccount {
    ...
    async saveManyResult(account, params, maxGasPrice = 400) {
        ...
        return sendAptosTx(this.client, account, `${this.switchboardAddress}::oracle_save_result_action::run`, [
            ...
            jobs_checksums.map((checksum) => Hex.fromHexString(checksum).toUint8Array()),
...
export class Permission {
    ...
    static async init(client, account, params, switchboardAddress) {
        const tx = await sendRawAptosTx(client, account, `${switchboardAddress}::permission_init_action::run`, [
            BCS.bcsToBytes(TxnBuilderTypes.AccountAddress.fromHex(params.authority.toString())),
            BCS.bcsToBytes(TxnBuilderTypes.AccountAddress.fromHex(params.granter.toString())),
            BCS.bcsToBytes(TxnBuilderTypes.AccountAddress.fromHex(params.grantee.toString())),
        ]);
...
export async function createFeedTx(client, sender, params, switchboardAddress) {
    const seed = params.seed ? params.seed : Account.generate().accountAddress;
...
export function bcsAddressToBytes(hexStr) {
    return BCS.bcsToBytes(TxnBuilderTypes.AccountAddress.fromHex(hexStr.toString()));
}
