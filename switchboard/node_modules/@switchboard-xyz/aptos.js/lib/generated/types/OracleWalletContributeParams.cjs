"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.OracleWalletContributeParams = void 0;
const common_1 = require("@switchboard-xyz/common"); // eslint-disable-line @typescript-eslint/no-unused-vars
class OracleWalletContributeParams {
    oracleAddr;
    queueAddr;
    loadAmount;
    constructor(fields) {
        this.oracleAddr = fields.oracleAddr;
        this.queueAddr = fields.queueAddr;
        this.loadAmount = fields.loadAmount;
    }
    toJSON() {
        return {
            oracleAddr: this.oracleAddr.toString(),
            queueAddr: this.queueAddr.toString(),
            loadAmount: this.loadAmount.toString(),
        };
    }
    static fromJSON(obj) {
        return new OracleWalletContributeParams({
            oracleAddr: obj.oracleAddr,
            queueAddr: obj.queueAddr,
            loadAmount: new common_1.BN(obj.loadAmount),
        });
    }
    toMoveStruct() {
        return {
            oracle_addr: this.oracleAddr.toString(),
            queue_addr: this.queueAddr.toString(),
            load_amount: this.loadAmount.toString(),
        };
    }
    static fromMoveStruct(obj) {
        return new OracleWalletContributeParams({
            oracleAddr: obj.oracle_addr,
            queueAddr: obj.queue_addr,
            loadAmount: new common_1.BN(obj.load_amount),
        });
    }
}
exports.OracleWalletContributeParams = OracleWalletContributeParams;
